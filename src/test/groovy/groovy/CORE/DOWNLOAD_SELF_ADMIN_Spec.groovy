...
import groovy.US.DOWNLOAD_SELF_ADMIN
import net.minidev.json.parser.JSONParser
import spock.lang.Specification

class DOWNLOAD_SELF_ADMIN_Spec extends Specification{
	def "getSelfAdminExcel"() {
		given:
		...
		domain.addFacts("request", [(RequestSegment.PathParams.name()):[tenantId:'SMD',groupNumber:'12345'], \
			...
			tenantId:'SMD',(RequestSegment.Body.name()): ["billNumber":"12348912345",groupNumber:'12345']])
		
		
		
		when:
		DOWNLOAD_SELF_ADMIN.newInstance().execute(domain)
	...
	def "getSelfAdminExcel Exception"() {
		given:
		def domain = new WorkflowDomain()
		def data=getTestData("smdBillingSelfadminBillProfile")
		def groupData=getTestData("groupDetails")
	...
	def "getSelfAdminExcel Data Not Found"() {
		given:
		def domain = new WorkflowDomain()
		def data=getTestData("smdBillingSelfadminBillProfile")
		def groupData=getTestData("groupDetails")
